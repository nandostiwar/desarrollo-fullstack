...
function VistaAdmin({ userRole }) {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:3000/api/users') // Asegúrate de que la ruta coincida con la definida en el backend
      .then((response) => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:3000/api/products') // Asegúrate de que la ruta coincida con la definida en el backend
      .then((response) => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:3000/api/sells') // Asegúrate de que la ruta coincida con la definida en el backend
      .then((response) => {
  ...
  const handleDeleteProduct = (productName) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // Si el usuario confirma la eliminación, realiza la solicitud DELETE
        axios
          .delete(`http://localhost:3000/api/products/${productName}`)
          .then((response) => {
            if (response.data.success) {
              // El usuario se eliminó con éxito, puedes realizar acciones adicionales si es necesario
  ...
  const handleCreateUser = () => {
    Swal.fire({
      ...
      preConfirm: () => {
        ...
        axios
          .post('http://localhost:3000/api/crearUsuario', { username, password, role, age, id })
          .then((response) => {
            if (response.data.success) {
  ...
  const handleCreateProduct = () => {
    Swal.fire({
      ...
      preConfirm: () => {
        ...
        axios
          .post('http://localhost:3000/api/crearProducto', { productName, price, id })
          .then((response) => {
            if (response.data.success) {
  ...
  return (
    <div>
      <Header1 titulo_principal="Bienvenido a la plataforma, Administrador" />
      <div>
        {showUsertable && (
